...
public class UserRepository : IUserRepository
{
    ...
    {
        return Task.Run(() => DbContext.Educations.First(ed => ed.EducationId.Equals(updatedEducation.Id)));
    }

    public async Task<User> GetUserByExecutorIdAsync(int id)
    {
        var t = await DbContext.Executors.Where(executor => executor.ExecutorId == id).FirstAsync();
        return await GetUserByIdAsync(t.UserId);
    }

    public async Task<User> GetUserByCustomerIdAsync(int id)
    {
        var t = await DbContext.Customers.Where(executor => executor.CustomerId == id).FirstAsync();
        return await GetUserByIdAsync(t.UserId);
    }

    public async Task<User> GetCommonUserByExecutorIdAsync(int id)
    {
        return await DbContext.Users.Where(user => user.Executor.ExecutorId == id)
            .Include(user => user.UserRoles)
            .Include(user => user.RefreshToken)
            .FirstAsync();
    }
    ...
    {
        return await DbContext.Users.Where(user => user.Customer.CustomerId == id)
            .Include(user => user.UserRoles)
            .Include(user => user.RefreshToken)
            .FirstAsync();
    }
