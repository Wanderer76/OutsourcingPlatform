...
public class ExecutorRepository : IExecutorRepository
{
    ...
    public async Task<bool> IsExecutorConnectWithCustomer(Customer customer, int executorId)
    {
        return await DbContext.Responses
            .Where(response => response.Order.Customer.CustomerId == customer.CustomerId)
    ...
    {
        return await DbContext.Executors
            .Where(executor => executor.ExecutorId == executorId)
            .Include(executor => executor.User)
            .FirstAsync();
    ...
    {
        return await DbContext.Responses.Where(response => response.ExecutorId == executorId && response.IsCompleted).CountAsync();
    }
