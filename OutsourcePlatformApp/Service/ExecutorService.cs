...
public class ExecutorService
{
    ...
    public async Task FinishProject(Executor executor, int orderId)
    {
        var order = await orderRepository.GetOrderByIdAsync(orderId);
        var userResponse = order.Responses.FirstOrDefault(response =>
            response.ExecutorId == executor.ExecutorId && response.IsAccept == true);
        if (userResponse == null)
            throw new ArgumentException("Пользователь не записан на проект");
        if (order.IsCompleted)
            throw new ArgumentException("Сожалеем, но заказчик уже завершил проект. Не переживайте, Ваша работа была проделана не зря, ведь вы всегда можете загрузить его в портфолио :)");
