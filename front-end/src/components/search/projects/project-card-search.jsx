...
const ProjectCardSearch = (props) => {
    ...
    let categories = props.info.orderCategories.categories.map((category) => {
        return <div className="field-tag" key={"categoryProject-"+props.info.orderId+"-"+category.id}>{category.name}</div>
    })
                ...
                <div className="project-params">
                    ...
                    <div className="row">
                        <img className="project-card-icon" src={deadlineIcon}
                             alt="Иконка дедлайна"/>
                        <p className="deadline">Дедлайн: <span>{new Date(props.info.deadline).toLocaleString("ru", {year: 'numeric', month: 'long', day: 'numeric'})}</span></p>
                    </div>
                </div>
                <div className="project-card-btns">
                    <input className="card-button blue" type="button" onClick={()=> {window.location.href = "/tasks/" + props.info.orderId}} value="Поcмотреть"/>
                    {button}
                </div>
            </div>
        </div>
