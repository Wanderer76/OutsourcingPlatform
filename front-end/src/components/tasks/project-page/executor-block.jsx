...
const ExecutorBlock = (props) => {
    ...
    function getMore() {
        console.log(props)
        let executors = props.executors.slice(offset * limit, (offset+1) * limit).map((executor) => {return <ExecutorCard info={executor} orderId={props.orderId} isCompleted={props.isCompleted} key={executor.executorId}/>})
        setOffset(offset + 1)
    ...
    return(
        <section className="project-contractors">
            <div className="project-contractors subtitle-row">
                <input className="project-contractors show-hide-list" type="button" id="contractors-btn" onClick={(e) => {
                    setVisible(!visible);
